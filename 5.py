...
class GameLauncher(QMainWindow):
    ...
    def launch_game(self):
        ...
        # Windowed mode arguments if enabled (comprehensive set)
        if self.windowed_check.isChecked():
            # Standalone flags
            cmd.extend(["-windowed", "-window", "-sw", "-nofullscreen", "-popupwindow"])
            # Flags that take a value (0/false)
            cmd.extend(["-fullscreen", "0", "-fullscreen", "false"])
            cmd.extend(["-screen-fullscreen", "0"])
            cmd.extend(["-window-mode", "windowed"])
            # Some engines use -adapter (with 0 for windowed? unlikely, but harmless)
            # Adding -adapter 0 might force windowed on some systems? better omit.
